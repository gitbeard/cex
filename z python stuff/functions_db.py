...
def db_insert_po(data):
    sql = 'INSERT INTO `purchase_orders`(`po_number`, `company_id`, `line_items`, `total_items`, `total_price`, `status`, `date_placed`, `date_ready`, `date_shipped`, `date_received`) \
            VALUES (\'%(po_number)d\', \'%(company_id)d\', \'%(line_items)d\', \'%(total_items)d\', \'%(total_price)d\', \'%(status)d\', \'%(date_placed)s\', \'%(date_ready)s\', \'%(date_shipped)s\', \'%(date_received)s\')' % data
    dbinsert_inv(sql) 

def db_insert_po_items(data):
    sql = 'INSERT INTO `purchase_order_items`(`po_id`, `line_number`, `item_number`, `part_number`, `part_description`, `quantity`, `unit_price`, `extended_price`, `status`) \
            VALUES (\'%(po_id)d\', \'%(line_number)d\', \'%(item_number)d\', \'%(part_number)s\', \'%(part_description)s\', \'%(quantity)d\', \'%(unit_price)f\', \'%(extended_price)f\', \'%(status)d\')' % data
    dbinsert_inv(sql) 

def dbselect_inv(table, where):
    sql = 'SELECT * FROM `' + table + '`'
    if(where != ''):
        sql = sql + ' WHERE ' + where
    #print(sql)
    return dbquery_inv(sql)
